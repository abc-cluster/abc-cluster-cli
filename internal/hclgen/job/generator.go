...
func appendStageTask(groupBody *hclwrite.Body, name, hook, manifest, manifestFile string, s StagingSpec) {
	...
	// The declarative s5cmd commands-file (generated by internal/jobstage). A
	// manifest with no commands (e.g. a stage-in with zero --in inputs — just a
	// header comment) is skipped: the template isn't written and the task only
	// ensures the staged mirror exists. Running s5cmd on it would be a pointless
	// no-op at best and, before this, aborted the job outright (B12).
	hasCmds := manifestHasCommands(manifest)
	if hasCmds {
	...
	// area. Referencing $NOMAD_TASK_DIR/local/<file> double-counted "local" →
	// /local/local/stage-in.txt: no such file, aborting the job before main (B12).
	// Match the main-task convention (ociTaskScriptArg) and drop the extra "local/".
	var shCmd string
	if hasCmds {
		shCmd = fmt.Sprintf(`mkdir -p "%s" && cd "%s" && %s %srun "$NOMAD_TASK_DIR/%s"`,
			s.DestRoot, s.DestRoot, s.S5cmdPath, s5cmdGlobal, manifestFile)
	} else {
		// Nothing to transfer — just ensure the staged mirror exists for main.
		shCmd = fmt.Sprintf(`mkdir -p "%s"`, s.DestRoot)
	}
	cfgBody.SetAttributeValue("args", cty.ListVal([]cty.Value{
		cty.StringVal("-c"),
		cty.StringVal(shCmd),
...
// manifestHasCommands reports whether an s5cmd commands-file contains at least
// one real command (a non-blank, non-comment line). A stage-in manifest with no
// declared inputs is just a header comment; a task running it should be a no-op.
func manifestHasCommands(manifest string) bool {
