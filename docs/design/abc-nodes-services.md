...
bash deployments/abc-nodes/experimental/scripts/init-supabase-secrets.sh  # generates JWTs
```
...
Vault is sealed on restart. Keep `experimental/acl/vault-keys.env` (generated by `init-vault.sh`) in a safe location — it contains the unseal keys and root token.
