...
| `contexts.<name>.cluster_type` | `abc-seedling` (canonical short form). Long-form `abc-cluster-seedling` and the `-tended` variant are accepted aliases; legacy `abc-nodes` / `abc-seed` resolve to the new canonical on read. Used by tier-default capability seeding. |
