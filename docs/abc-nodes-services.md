...
Traefik is the single HTTP entry point for all services. It runs as a raw_exec process (not a container) so it can bind to privileged port 80 and reach backend services at `127.0.0.1:<port>` via the host network stack.
