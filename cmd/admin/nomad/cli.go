...
func newCLICmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:               "Run the local nomad binary as a preconfigured alias. Nomad address, token, and region are resolved from the active abc config context when not provided via flags. For contexts with cluster_type abc-nodes, admin.abc_nodes.nomad_namespace (e.g. default) is exported as NOMAD_NAMESPACE only when that variable is not already set in the environment. Prefer the passthrough separator: `abc admin services nomad cli -- <nomad-args...>` so argv matches the upstream `nomad` CLI exactly (e.g. `... nomad cli -- job status -short`). Optional leading `--binary-location <path>` before `--`. The lone subcommand `abc admin services nomad cli setup` is handled by abc (installs nomad + abc-node-probe into ~/.abc/binaries, or use `abc admin services cli setup` for tailscale and rclone too); it is not forwarded to the nomad binary. Without `--`, tokens after any `--binary-location` pairs are still forwarded to nomad for compatibility.",
