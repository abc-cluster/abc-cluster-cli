...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  abc job run bwa-align.sh --output-file bwa-align.hcl`,
	...
	cmd.Flags().String("shell", "",
		"Script interpreter override. Accepts:\n"+
			"  bare name  (bash, sh, dash, zsh, fish, …)  → resolved to /bin/<name>\n"+
			"  abs path   (/usr/local/bin/fish, /bin/dash) → used verbatim\n"+
			"Defaults: /bin/sh for OCI drivers (docker, containerd-driver, podman,\n"+
			"singularity) so jobs work on alpine / distroless / minimal images;\n"+
			"/bin/bash for host-side drivers (exec, exec2, raw_exec, java, hpc-bridge).\n"+
			"The shell must exist inside the target image — the kernel reports a clear\n"+
			"ENOENT at exec time if the path is missing.")
...
func runJob(cmd *cobra.Command, args []string) error {
	...
	if isBuiltInHelloCluster {
		...
		// Stamp submission metadata into meta block. Includes the user-
		// identity correlation context (abc_user_* keys + abc_workspace +
		// abc_cli_version + abc_submitted_at) using the SAME key shape that
		// `abc pipeline run` emits, so a single jurist / xtdb / Grafana
		// query can pivot across job + pipeline runs by the same fields.
		// Canonical definition lives in
		// `internal/hclgen/pipeline/generator.go` (Identity.MetaMap()) —
		// keep the keys here in sync when that surface evolves.
		if spec.Meta == nil {
			spec.Meta = map[string]string{}
		}
