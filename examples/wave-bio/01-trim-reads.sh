...
#
# Uses wave-exec: Wave (Seqera) builds a container from environment.yml once
# and reuses it for every subsequent job — no per-job conda install overhead.
# The container is guaranteed pullable before this task starts (prestart task
# runs wave --await).
#
# Required meta keys:
